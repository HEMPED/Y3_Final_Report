...
\begin{itemize}
    ...
    \item \textbf{Core Objective 4:} Implement Per-Option Delegation into Vodle.
...
Delegation chains are resolved internally within the browser and are never publicly exposed. Additionally, delegations only becomes active when the delegate explicitly accepts the invitation -- ensuring that no information about a user's voting intentions is shared without their consent.
...
The project was managed effectively through iterative development, regular reassessment of priorities, and structured risk management. Initial planning enabled steady progress, while flexibility allowed for the successful descoping of optional extensions when necessary. Identified risks, including time constraints and unforeseen complexity of delegation features, were managed without impacting the quality or achievement of the core objectives. Overall, the project achieved its primary aims within the planned timeframe and established a strong basis for future development, discussed in the next chapter.
