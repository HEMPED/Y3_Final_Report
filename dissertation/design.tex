...
If a cycle is detected during delegation acceptance, the system prevents the delegation and notifies the user why (see Figure~\ref{fig:del-accept-cycle}).
...
To enhance the resilience of delegation relationships and reduce the risk of vote loss, ranked delegation was introduced. This extension allows users to specify multiple trusted delegates in a preferred order, ensuring that their vote remains effective even if their primary delegate abstains or becomes unavailable. Ranked delegation is resolved according to the MinSum rule, favouring the most preferred available delegate while preserving transitive consistency.
...
\begin{figure}[H]
  \centering
  % \includegraphics[width=0.8\textwidth]{figures/per-option-delegation-invite-placeholder.png}
...
\begin{figure}[H]
  \centering
  % \includegraphics[width=0.8\textwidth]{figures/per-option-delegation-management-placeholder.png}
...
%   \item talk about altering maps to make them nested map - need to take care to serialise.
...
\begin{figure}[h]
    \centering
    % \includegraphics[width=0.8\textwidth]{figures/trust-slider-placeholder.png}
...
\begin{figure}[h]
    \centering
    % \includegraphics[width=0.8\textwidth]{figures/expert-mode-placeholder.png}
...
In summary, the use of an epsilon threshold ensures that minor floating-point inaccuracies do not trigger unnecessary iterations, while the recursion depth limit guarantees the system remains performant and stable even in complex delegation graphs.
...
\section{Summary}
The revised core delegation model established a synchronised, cycle-free system for managing vote delegations in vodle. By maintaining global delegation maps and enforcing efficient cycle prevention at the point of delegation acceptance, the system guarantees consistent vote resolution across all clients. Explicit invitation flows and integrated user interface updates uphold user autonomy and improve the transparency and usability of the delegation process, providing a robust foundation for the advanced delegation mechanisms introduced in the following sections.