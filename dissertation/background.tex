...
Google Votes was an experimental implementation of liquid democracy deployed internally at Google. It was built on top of the company's corporate Google+ social network and ran over a three-year period, from 2012 to 2015 \citep{hardt_google_2015}. The platform allowed users to either vote directly or delegate their vote to other employees, with support for transitive delegations. Delegations were category-specific, enabling users to assign trusted colleagues to vote on specific topics such as "food", "coding", or "events".

One of the goals of Google Votes was to reduce barriers to participation while encouraging meaningful contributions. Users could override their delegation at any time and cast a vote directly. Delegation features were introduced gradually and supported through user-friendly interactions, including voting power estimates, delegation advertisements, and notifications whenever a delegate voted on someone's behalf.

Google Votes also addressed transparency through what the authors call the ``Golden Rule of Liquid Democracy'': if a user delegates their vote, they must be able to see how it is used. All votes were visible to the voting group, and users were notified when a delegate voted on their behalf. This helped maintain accountability and encouraged users to monitor their representation.

The system supported various voting styles, including ranked, approval, and score voting, with tally methods like the Schulze and Borda count. One notable application was the ``Mountain View Microkitchen Food Fair'', which received over 62,000 votes from 4,600 users in a single week. While delegated votes accounted for only around 3.6\% of total votes cast, their use grew over time and played a meaningful role in decisions such as food selection and event planning.

Despite not being made public, Google Votes demonstrated that liquid democracy could be applied successfully within a corporate environment, offering a scalable and adaptable model for collaborative decision-making \citep{hardt_google_2015}.
\section{vodle}
...
Each voter rates an option from $0$ to $100$ ($x$), representing their willingness to approve that option if and only if $<x\%$ of users do not approve that option. Therefore, a rating of $0$ means ``do no approve no matter what'' and a rating of $100$ means ``approve no matter what'' or ``always approve''. This can be seen visualised in the illustration below.
