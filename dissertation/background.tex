...
\begin{figure} [H]
    ...
    \caption{Screenshot of popup window for creating a delegation link.}
    \label{fig:deldialog_initial}
\end{figure}

\textbf{Insert Code Snippet:} Function for creating a delegation link and storing it in the CouchDB database.
...
\textbf{Insert Screenshot:} Acceptance screen presented to User B.
...
\begin{itemize}
    \item User A sees a message indicating that their vote has been delegated and can no longer interact with the voting interface for that poll.
    
    \textbf{Insert Screenshot:} User A's view after delegation.

    \item User B is notified that they are voting on behalf of another user. The interface reflects this with visual indicators showing the number of users their vote is used for.
    
    \textbf{Insert Screenshot:} User B's view while acting as a delegate.
\end{itemize}
...
Although the current system includes logic intended to prevent delegation cycles, it does not function reliably in all cases. While it can block simple cases like direct self-delegation, more complex cycles involving multiple users are not consistently detected. For instance, in a chain where User A delegates to B, B to C, and C back to A, the system may fail to recognise the cycle depending on \dots

\textbf{Insert Code Snippet:} Example showing the existing cycle detection logic and where it may fail.
