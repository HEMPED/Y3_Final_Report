...
\subsection{CouchDB Storage and Write Constraints -- may remove/ shorten}
...
This section describes the design and implementation of ranked delegation in vodle, addressing Objective 2 of the project. The ranked delegation system allows users to specify fallback delegates in case their primary delegate is unavailable -- due to being a part of a delegation cycle or abstaining. 

\subsection{Overview}

For the implementation in vodle, users can specify a ranked list of up to three delegates, which are poll specific. To resolve delegation paths, the system applies the MinSum delegation rule (see Section~\ref{subsec:background_ranked_delegation}), ensuring that votes are routed along paths that minimise the total delegation cost based on rank.

Key features of the ranked delegation implementation include:
\begin{itemize}
    \item A user interface for selecting and reordering ranked delegates.
    \item Validation to prevent assigning the same user multiple times.
    \item Dynamic resolution of delegation paths using the MinSum rule.
    \item Cycle detection to prevent invalid delegation graphs.
\end{itemize}

\subsection{Data Structures}

Ranked delegations are recorded in the \texttt{direct\_delegation\_map}, mapping each user ID to a list of their ranked delegates. Each delegate is represented as a triple -- \texttt[delegation\_id, rank, status].
where:
\begin{itemize}
    \item \texttt{delegation\_id} is the identifier of the delegate.
    \item \texttt{rank} is an integer representing the delegate's rank (1 being the most preferred delegate).
    \item \texttt{status} tracks whether the delegation has been accepted, rejected, is pending, or active (currently part of a delegation path).
\end{itemize}

For example, suppose User A ranks User B first, User C second, and User D third. Users B and C accept the delegation, while User D does not respond. If no other delegations exist, the delegation map would be as follows:
\begin{figure}[H]
  \begin{minted}{python}
  ...
  \end{minted}
  \caption{Example of a \texttt{direct\_delegation\_map}, used in ranked delegation.}
\end{figure}

As no other delegations exist, MinSum will choose the path with the lowest rank ($A\to B$) and activate all edges along the path (status has a value of 2).

\subsection{Delegation Resolution Using MinSum}

Whenever a delegation is accepted, rejected, or revoked, the delegation paths must be recomputed using the MinSum rule: 

\begin{enumerate}
    \item Possible paths from each voter to a casting voter are generated by traversing the ranked delegation graph.
    \item Each edge in the graph is assigned a cost equal to the delegation rank (first-ranked delegate costs 1, second-ranked costs 2 and the third-ranked costs 3).
    \item The path with the minimal total sum of ranks is selected as the resolution path.
    \item If multiple paths have the same minimal cost, userID is used as a tie breaker.
    \item If no casting voter is reachable from a particular user, they must vote themselves.
\end{enumerate}

\subsection{Cycle Prevention}
Delegation cycles are handled during the generation of delegation paths. When a path is being expanded, the system checks whether the next node has already been found in the path. In the case that it has, the search along that branch is terminated.
...
\end{minted}
\caption{Code snippet to prevent cycles from being explored, \texttt{did} refers to the next delegate along a path.}
\end{figure}

\subsection{User Interface}

The user interface for ranked delegation was designed to be simple and mobile-friendly:
\begin{itemize}
    \item Users can select up to three delegates using a drag-and-drop list.
    \item Duplicates are automatically detected and prevented.
    \item Delegation rankings can be edited at any time through a dedicated interface accessible when voting.
    \item Visual indicators show the acceptance or rejection status of each delegation.
\end{itemize}

This design ensures that ranked delegation remains accessible to users with varying levels of technical familiarity.

\subsection{Challenges and Solutions}

Several challenges were encountered during the development of the ranked delegation system. These challenges, along with brief notes for future expansion, are listed below:

\begin{itemize}
    \item \textbf{Efficient MinSum Pathfinding:} Finding the minimal-sum paths through a dynamic delegation graph without excessive computational cost.
    
    [Placeholder: explain how this was solved]

    \item \textbf{Handling Changes to Rankings:} Ensuring that reordering delegates dynamically updated stored mappings and did not leave stale data.
    
    [Placeholder: explain how this was solved]

    \item \textbf{Cycle Detection Without Server-Side Validation:} Detecting and preventing delegation cycles entirely on the client side.
    
    [Placeholder: explain how this was solved]

    \item \textbf{Maintaining UI Simplicity:} Providing ranked delegation functionality without overwhelming users, especially on mobile.
    
    [Placeholder: explain how this was solved]
\end{itemize}

Each of these challenges influenced the final design and required careful balancing of performance, robustness, and usability.
