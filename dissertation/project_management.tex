...
\subsection{Actual Timeline vs Planned Timeline -- TODO}
While the original project plan offered a clear sequence for implementing each objective, the actual progression of the project deviated in several key areas due to technical complexity, user interface considerations, and evolving priorities. The Gantt chart below (Figure~\ref{fig:actual_gantt}) summarises the planned versus actual timeline for each major objective. The following subsection provides a breakdown of the key deviations and the reasons behind them.
...
\begin{itemize}
    \item Mentioned swapping 3,4 already
    \item objective 1 delayed due to time spent understanding the existing codebase -- especially the database schema and how to implement the delegation model in a way that would be compatible with it.
    \item 
\end{itemize}
