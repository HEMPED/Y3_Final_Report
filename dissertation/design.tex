...
As discussed in Section~\ref{subsec:delegation_cycles}, cycle prevention is critical to ensure that delegated votes are eventually cast by a valid voter. If a cycle forms -- for example, if A delegates to B, B to C, and C back to A -- none of the votes in the cycle can be resolved, effectively nullifying them. Detecting and preventing such structures is therefore a fundamental requirement for maintaining the integrity of the vote resolution process.

The original implementation in vodle included a rudimentary form of cycle detection, but this logic was applied inconsistently. Since each browser session maintained a local view of the delegation graph, users could unknowingly create cycles that were not detected in real time or that only became apparent on other devices. This subsection explores the limitations of the previous system and presents the improved cycle-checking mechanism developed as part of this project.

\Huge\textbf{TALK ABOUT THE CYCLE CHECKING LOGIC IN THE OLD SYSTEM}\large
