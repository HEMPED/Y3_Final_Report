...
Due to the distinct design goals of each delegation mechanism, they were developed as separate modes rather than fully interoperable features. When creating a poll, users must select a single delegation mode -- standard delegation, ranked delegation, weighted delegation, or per-option delegation -- which then determines the available delegation features during voting.

This separation also reflects the project's longer-term intention: in the final version of vodle, only one delegation mechanism will be retained (see Chapter~\ref{ch:conclusions} for more details). Exploring multiple modes in the development environment allowed evaluation of different approaches, but mixing modes within a single poll was deliberately avoided. Allowing poll creators to combine delegation features could introduce bias by letting them selectively favour mechanisms advantageous to themselves. By enforcing a single, consistent delegation mode per poll, vodle preserves fairness and neutrality.
...
This architecture has several implications for the design and implementation of the delegation mechanisms. These arise especially due to the absence of a traditional data processing backend. The following points summarise the key considerations:
\begin{itemize}
  \item All vote delegation logic, including transitive resolution, cycle detection, and weighted delegation calculations, must be executed in the browser, placing constraints on performance and requires careful optimisation of algorithms used.
...
Prior to the inception of this project, vodle included a partially implemented delegation system. Although disabled due to critical flaws, the system introduced several foundational ideas and structures that influenced the final design. This section introduces the key data structures used in the original model, explains the intended delegation flow, and analyses why a redesign was necessary.
...
\begin{itemize}
    ...
    \item \textbf{effective\_delegation\_map}: This map stored the final casting voter for each user by resolving the full transitive chain of delegations. For example, if user A delegated to user B and user B delegated to user C, the effective delegate of user A was user C.
...
\begin{enumerate}
    ...
    \item A magic link\footnote{A magic link is a secure, single-use hyperlink that authenticates a user or authorises an action without requiring manual credential entry. In this context, it contains a delegation request payload, allowing the intended delegate to accept or reject the invitation upon clicking the link.} containing this information was shared with the intended delegate.
...
\subsubsection{Overriding Delegated Vote}

The original vodle implementation included basic support for per-option overriding of delegated votes: when a user had active delegations, a banner such as ``\texttt{B controls some of your waps as your delegate}'' was displayed, and users could manually toggle between using their delegate's rating or submitting their own per poll option. However, the existing override functionality was inconsistent and often failed to update the UI correctly when delegation states changed. This override system was revised and extended to integrate reliably with the redesigned core delegation model, as well as the new ranked, weighted, and per-option delegation mechanisms developed during this project.

The toggle for each poll option was set by the \texttt{rate\_yourself\_toggle[option\_id]} array. When set to \texttt{true}, the user's own rating was applied; otherwise, the delegated rating was used. The number of options under delegation was tracked using the following logic:
...
This project also introduced a new user interface for managing existing delegations during an active poll. This work supports functional requirements FR4 (provide users with a clear view of their ranked delegation choices, including the ability to alter their rankings at any time) as specified in the requirements (see Section~\ref{req_ranked}). Users can view their current delegations, revoke them, or reorder them via drag-and-drop. This interface is provided through an information dialog, as shown in in Figure~\ref{fig:info_rank} and can be accessed by pressing the information button shown in in Figure~\ref{fig:info_button_rank}.
...
\section{Implement Per-Option Delegation into Vodle}
\label{sec:design_per_option_delegation}

To better reflect the varying expertise and trust relationships that arise across different decision topics, per-option delegation was introduced into vodle. This feature allows users to assign different delegates for individual poll options, providing significantly greater flexibility than poll-wide delegation models.
...
\begin{itemize}
  ...
  \[
  \begin{aligned}
    \texttt{option\_id} &\rightarrow \texttt{user\_id} \\
                     &\rightarrow \text{list of users who delegate to this user (directly or indirectly)}
\end{aligned}
  \]
\end{itemize}
...
The system builds upon the trust matrix model introduced in Section~\ref{subsec:background_weighted_delegation} but adapts it to vodle's architecture, where all computation must be efficiently performed within the browser.

\subsection{Data Structures}

To accommodate weighted delegation, the \texttt{direct\_delegation\_map} was adapted to store trust values instead of ranks:
...
\begin{figure}[H]
  ...
  \caption{Expert mode enabled. Users can edit the trust of their delegations and save.}
  \end{subfigure}
...
The central function responsible for computing updated ratings is \\\texttt{update\_effective\_votes()}, which is called whenever users modify their self-assigned ratings of poll options or delegation relationships (changing trust, adding a delegate, or removing a delegate):
...
\section{Database Validation Updates}
To support liquid democracy features in vodle, minimal changes were made to the underlying CouchDB schema. Instead of introducing new document types, the system reuses existing poll structures by storing delegation-related data as special poll documents.

The only database-level modification was an update to the \texttt{validate\_doc\_update.js} file, which enforces document-level access control rules in CouchDB.

Specifically:

\begin{itemize}
\item A new exception was introduced allowing users to create or update documents whose \texttt{\_id} ends with \texttt{direct\_delegation\_map}, \texttt{inverse\_indirect\_map}, or \texttt{waps}.

\item These exceptions permit storing delegation mappings associated with a poll while maintaining strict restrictions on other poll documents.

\item Access control was preserved: only users associated with the poll (voters) are allowed to create or modify these documents, and updates remain prohibited after the poll's due date.
\end{itemize}

The rest of the vodle database structure, including user and poll document formats, remains unchanged. This approach ensured backward compatibility and minimal disruption to existing data flows. The following code snippet shows the relevant section of the \texttt{validate\_doc\_update.js} file:
...
This chapter described the design and implementation of the four delegation modes integrated into vodle, with particular attention to maintaining client-side efficiency, consistency under CouchDB's document constraints, and user-driven control of delegation. The solutions developed establish the basis for evaluating the system's correctness and performance, as explored in the next chapter.