...
\subsection{Limitations of the Original System}
...
The recipient could then accept (see Figure~\ref{fig:delegation-flow-accept}) or reject (see Figure~\ref{fig:delegation-flow-reject}) the request by submitting a signed response (\texttt{del\_response}) using the provided private key. This created a delegation agreement object (\texttt{del\_agreement}) locally on both clients.

% Figure~\ref{fig:delegation-flow-accept} shows this successful flow:
% \begin{itemize}
%   \item User A shares a delegation link with User B.
%   \item User B accepts the invitation.
%   \item User A's vote is now controlled by User B.
% \end{itemize}

% If the delegate rejects the invitation, both users proceed independently, as shown in Figure~\ref{fig:delegation-flow-reject}.

% \begin{enumerate}
%   \item User A (delegator) generates a delegation link and shares it with User B (delegate).
%   \item User B is prompted to accept or reject this invitation.
%   \item If accepted (see Figure~\ref{fig:delegation-flow-accept}), User B's vote is used for User A. If rejected (see Figure~\ref{fig:delegation-flow-reject}), both vote independently.
% \end{enumerate}
