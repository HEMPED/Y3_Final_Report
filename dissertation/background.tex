...
%Each voter distributes their full voting power across several proxies. The model computes a global equilibrium by recursively resolving delegations until only casting votes remain. However, it enforces $t_{i,i} = 0$, meaning voters who use the mechanism entirely forfeit their direct say. Moreover, the system does not limit how far delegated weight can propagate, allowing long chains to accumulate negligible contributions that still affect the outcome.
